...
function editBlotter2($connn, $bno, $fname, $complainant, $complained, $loc, $dof, $status, $BI, $pin, $sstat,$id,$userID) {
	...
	$sql1 = "SELECT user_email FROM users WHERE user_id = ?;";
	$stmt1 = mysqli_stmt_init($connn);
	...
	if ($emal !== "") {
		...
		header('location: BlotterrequestmoreEmail');
		exit();
	}
	header("location: Blottermore");
		exit();
...
function editDocReq11($connn, $fname, $daterequest, $dt, $purok, $BI, $dof, $status, $id, $emaiil)  {
	...
	$_SESSION['emal'] = $emaiil;
	$_SESSION['dt'] = $dt;	
	$_SESSION['de'] = "Approved";	
...
function positionCheck($connn, $com, $position) {
	 $sql = "SELECT * FROM officials WHERE committee = ? AND position= ?";
	$stmt = mysqli_stmt_init($connn);
	if (!mysqli_stmt_prepare($stmt, $sql)) {
		header("location: sign_up?error=stmtfailed");
		exit();
	...
	mysqli_stmt_bind_param($stmt, "ss", $com, $position);
	mysqli_stmt_execute($stmt);
...
function fingerprint($connn, $userid) {
	$sql3 = "SELECT * FROM users_browser WHERE user_id = ?;";
	$stmt3 = mysqli_stmt_init($connn);
